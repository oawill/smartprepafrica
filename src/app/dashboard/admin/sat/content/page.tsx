...
export default async function AdminSatContentPage({
  searchParams,
}: {
  searchParams: Promise<{ section?: string; status?: string; q?: string; page?: string; error?: string }>;
}) {
  ...
  const where: Prisma.SatContentWhereInput = {
    ...(params.section ? { section: params.section as never } : {}),
    ...(params.status ? { status: params.status as never } : {}),
    ...(q ? { prompt: { contains: q, mode: "insensitive" } } : {}),
  };

  const [total, items] = await Promise.all([
    prisma.satContent.count({ where }),
  ...
  function pageHref(overrides: Record<string, string | undefined>) {
    ...
    const merged = { q, section: params.section, status: params.status, ...overrides };
    for (const [k, v] of Object.entries(merged)) if (v) sp.set(k, v);
    return `/dashboard/admin/sat/content?${sp.toString()}`;
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">SAT Content</h1>
          <p className="mt-1 text-sm text-text-secondary">{total} total — {PAGE_SIZE} shown per page.</p>
        </div>
        <Link
          href="/dashboard/admin/sat/content/new"
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-b border-border last:border-0">
                    <td className="py-2 pr-4">{SAT_SECTION_LABELS[item.section]}</td>
                    <td className="py-2 pr-4 text-text-secondary">{item.domain}</td>
                    <td className="py-2 pr-4 font-mono text-xs text-text-secondary">{item.questionType}</td>
                    ...
                    <td className="py-2">
                      <Link
                        href={`/dashboard/admin/sat/content/${item.id}`}
