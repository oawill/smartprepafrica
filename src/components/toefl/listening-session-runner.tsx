...
import { saveListeningAnswer, submitListeningAttempt } from "@/app/international-exams/toefl/listening/actions";

// Deliberately does NOT include a transcript field — the parent server
// page never fetches/passes one during active practice, so it can't leak
// into this client bundle. Transcript only appears on the results page.
export type ListeningSessionItem = {
  itemId: string;
  audioUrl: string;
  prompt: string;
  options: unknown;
...
export function ListeningSessionRunner({ attemptId, items }: { attemptId: string; items: ListeningSessionItem[] }) {
  ...
  function select(optionKey: string) {
    ...
    startTransition(async () => {
      await saveListeningAnswer(current.itemId, optionKey);
    });
  }

  function submit() {
    startTransition(async () => {
      await submitListeningAttempt(attemptId);
    });
  }
